...
const UserAppointments = () => {
  ...
  const getUserAppointment = async () => {
    console.log(userid)
    try {
      const res = await axiosInstance.get('/api/user/getuserappointments', {
        headers: {
  ...
  const getDoctorAppointment = async () => {
    console.log(userid)
    try {
      const res = await axiosInstance.get('/api/doctor/getdoctorappointments', {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
        params: {
  ...
  const handleStatus = async (userid, appointmentId, status) => {
    try {
      const res = await axiosInstance.post('/api/doctor/handlestatus', {
        userid,
        appointmentId,
        status,
      },
  ...
  const handleDownload = async (url, appointId) => {
    try {
      const res = await axiosInstance.get('/api/doctor/getdocumentdownload', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem("token")}`,
        },
        params: { appointId },
